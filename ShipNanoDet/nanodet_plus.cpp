...
NanoDet::NanoDet(const char* param, const char* bin, AppConfig* appConfig)
{
    printf("start initializing NanoDet instance ...\n");
    // cache the appConfig
    memcpy(&mAppConfig, appConfig, sizeof(AppConfig));
...
std::vector<BoxInfo> NanoDet::detect(cv::Mat image, float nms_threshold)
{
    ncnn::Mat input;
    ...
    this->decode_infer(out, center_priors, mAppConfig.det_conf_thresh, results);
